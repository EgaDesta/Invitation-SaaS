...
export default function AdminTemplates() {
  ...
  const handleSave = async () => {
    ...
    if (editId) {
      const { error } = await supabase.from("invitation_templates").update({ name: form.name, category: form.category }).eq("id", editId);
      if (error) toast.error(error.message);
      else toast.success("Template diperbarui");
    } else {
      const { error } = await supabase.from("invitation_templates").insert({ name: form.name, category: form.category, is_public: true, template_data: {} });
      if (error) toast.error(error.message);
        ...
        <div className="flex items-center justify-between">
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
              <Button className="gap-2" onClick={() => { setEditId(null); setForm({ name: "", category: "wedding" }); }}>
              ...
              <CardContent className="p-4">
                ...
                <div className="flex gap-2">
                  <Button
                    size="sm"
                    variant="ghost"
                    onClick={() => { setEditId(t.id); setForm({ name: t.name, category: t.category }); setOpen(true); }}
                  >
